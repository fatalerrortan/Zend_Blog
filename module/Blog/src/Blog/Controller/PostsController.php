...
//        all posts template
        if(empty($pattern)){
            $allposts_load = $this->formatTargetPosts($this->postService->findAllPosts('', $category), true);
            if(!empty($category)){
                $buttonflag = "<input id='buttonflag' type='hidden' value='0' />";
            }else{$buttonflag = "<input id='buttonflag' type='hidden' value='1' />";}
        }else{
            $allposts_load = $this->formatTargetPosts($this->postService->findAllPosts('', '', $pattern), true);
            $buttonflag = "<input id='buttonflag' type='hidden' value='0' />";
        }
